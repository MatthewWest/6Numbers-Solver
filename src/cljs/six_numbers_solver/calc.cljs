(ns six-numbers-solver.calc
  (:require-macros [six-numbers-solver.macros :as macros]))
              ...
              (let [j (loop [j (- n 2)] ; begin H4
                        (if (and (< (a j) (b j))
                                 (if r
                                   ...
                                   true)
                                 (if s
                                   (<= (- s (b j) (macros/reify-bool (== (inc (a j)) (b j)))) (- n j))
                                   true))
                          j
                          (recur (dec j))))] ; end H4
                (if (zero? j) ;begin H5
                  ...
                  (let [a (update a j inc) ; end H5
                        x (when s
                            (- s
                               (+ (b j)
                                  (macros/reify-bool (= (a j) (b j))))))
                        [a b] (loop [a a
                                     b b
                                     i (inc j)
                                     current-max (+ (b j)
                                                    (macros/reify-bool (== (b j) (a j))))]
                                (cond
                                  (== i n) [a b]
...
(defn guesses [inputs]
  ...
  (let [operators [+ - * /]]
    (for [order (subsets inputs)
          oporder (selections operators (dec (count order)))]
